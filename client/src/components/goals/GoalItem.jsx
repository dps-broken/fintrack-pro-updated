...
const GoalItem = ({ goal, onEdit, onDelete, onOpenUpdateProgressModal }) => {
  ...
  return (
    <motion.div variants={itemVariants}>
      <Card
        className={`flex flex-col h-full border-2 ${isAchieved ? 'border-green-500 dark:border-green-400' : 'border-transparent'}`}
        hoverEffect={!isAchieved}
      >
        <CardBody className='flex-grow'>
          <div className='flex justify-between items-start mb-3'>
            <div className='flex items-center space-x-3'>
              ...
              <div>
                <h3
                  className='text-lg font-semibold text-text-light dark:text-text-dark truncate'
                  title={name}
                >
                  {name}
                </h3>
          ...
          <div className='flex flex-col sm:flex-row items-center sm:items-start space-y-4 sm:space-y-0 sm:space-x-4'>
            ...
            <div className='flex-grow space-y-1 text-sm text-center sm:text-left'>
              ...
              {deadline && (
                <p
                  className={`flex items-center justify-center sm:justify-start text-xs ${daysLeft < 0 && !isAchieved ? 'text-red-500' : 'text-text-muted-light dark:text-text-muted-dark'}`}
                >
              ...
              {description && (
                <p
                  className='text-xs text-text-muted-light dark:text-text-muted-dark pt-1 italic truncate'
                  title={description}
                >
                  {description}
                </p>
