...
const ProfilePage = () => {
  ...
  const settingsFormik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        const [themeResponse, emailPrefsResponse] = await Promise.all([
          themeUpdatePromise,
          emailPrefsUpdatePromise,
        ]);
  ...
  const fetchCustomCategories = useCallback(async () => {
    ...
    try {
      ...
      setCustomCategories(
        allCategories.filter((cat) => !cat.isPredefined && cat.user === user._id)
      );
    } catch (error) {
  ...
  const handleDeleteCategory = async (categoryId) => {
    if (
      window.confirm('Are you sure you want to delete this custom category? This cannot be undone.')
    ) {
      try {
  ...
  const handleDeleteAccount = async () => {
    if (
      ...
      if (
        window.confirm(
          'SECOND CONFIRMATION: This is your last chance. Are you sure you want to delete your account?'
        )
      ) {
    ...
    <div className='space-y-8'>
            ...
            <form onSubmit={profileFormik.handleSubmit} className='space-y-6'>
              ...
              <div className='flex justify-end'>
                <Button
                  ...
                  disabled={isLoadingProfile || !profileFormik.dirty}
                >
                  Save Profile
                </Button>
              </div>
            ...
            <form onSubmit={passwordFormik.handleSubmit} className='space-y-6'>
              ...
              <div className='flex justify-end'>
                <Button
                  ...
                  disabled={isLoadingPassword || !passwordFormik.dirty}
                >
                  Change Password
                </Button>
              </div>
          ...
          <CardBody>
            <form onSubmit={settingsFormik.handleSubmit} className='space-y-6'>
              <div>
                ...
                >
                  <PaintBrushIcon className='inline h-5 w-5 mr-2 align-text-bottom' />{' '}
                ...
                <Select
                  ...
                  value={themeOptions.find(
                    (opt) => opt.value === settingsFormik.values.themePreference
                  )}
                  onChange={(option) =>
                    settingsFormik.setFieldValue('themePreference', option.value)
                  }
              ...
              <div className='flex justify-end'>
                <Button
                  ...
                  disabled={isLoadingSettings || !settingsFormik.dirty}
                >
                  Save Settings
                </Button>
              </div>
      ...
      >
        <Card className='border-red-500 dark:border-red-600'>
          <CardHeader
            title='Danger Zone'
            className='bg-red-50 dark:bg-red-900/30 border-b-red-200 dark:border-b-red-700'
          />
          <CardBody>
            <div className='flex flex-col sm:flex-row justify-between items-center'>
              <div>
                <h4 className='text-md font-semibold text-text-light dark:text-text-dark'>
                  Delete Your Account
                </h4>
